layui.use(['layer','form','table'], function() {
    ...
    //监听工具条
    table.on('tool(abnormityTypeList)', function(obj){
        var data = obj.data;
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑选项",
                type : 2,
                content : "/customer/abnormityType/edit?id="+data.id,
        ...
        if(obj.event === "del"){
            layer.confirm("你确定要删除该选项么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/customer/abnormityType/delete",{"id":data.id},function (res){
                        if(res.success){
                            layer.msg("删除成功",{time: 1000},function(){
                                table.reload('abnormityType-table', t);
                            });
                        }else{
    ...
    t = {
        elem: '#abnormityType-table',
        even: true,
        url:'/customer/abnormityType/list',
        method:'post',
        ...
        cols: [[
           ...
           /* {field:'id',        title: 'ID'   },*/
            ...
            {title: '操作',fixed: 'right',  width:'15%',    align: 'center',toolbar: '#abnormityTypeBar'}
        ]]/*,
        done: function () {
    ...
    var active={
        addUser : function(){
            addIndex = layer.open({
                title : "添加选项",
                type : 2,
                content : "/customer/abnormityType/add",
        ...
        //批量删除
        deleteSome : function(){
            var checkStatus = table.checkStatus('abnormityType-table'),
            ...
            if(data.length > 0){
                ...
                layer.confirm("你确定要删除这些选项么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        ...
                        $.ajax({
                            type:"POST",
                            url:"/customer/abnormityType/deleteSome",
                            dataType:"json",
    ...
    form.on("submit(searchForm)",function(data){
        table.reload('abnormityType-table', {
            page: {curr: 1},
            where: data.field
        });
        return false;
