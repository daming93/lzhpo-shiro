layui.use(['layer','form','table'], function() {
    ...
    table.on('tool(brandList)', function(obj){
        ...
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑品牌",
                type : 2,
                content : "/item/brand/edit?id="+data.id,
                success : function(layero, index){
                    setTimeout(function(){
                        layer.tips('点击此处返回品牌列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
                        });
                    },500);
                }
        ...
        if(obj.event === "del"){
            layer.confirm("你确定要删除该品牌么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/item/brand/delete",{"id":data.id},function (res){
                        if(res.success){
                            layer.msg("删除成功",{time: 1000},function(){
                                table.reload('brand-table', t);
                            });
                        }else{
    ...
    t = {
        ...
        cols: [[
           ...
           /* {field:'id',        title: 'ID'   },*/
            ...
            {title: '操作',fixed: 'right',  width:'15%',    align: 'center',toolbar: '#brandBar'}
        ]]/*,
        done: function () {
    ...
    var active={
        ...
        //批量删除
        deleteSome : function(){
            var checkStatus = table.checkStatus('brand-table'),
                data = checkStatus.data;
            if(data.length > 0){
                console.log(JSON.stringify(data));
                layer.confirm("你确定要删除这些品牌么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        var deleteindex = layer.msg('删除中，请稍候',{icon: 16,time:false,shade:0.8});
                        $.ajax({
                            type:"POST",
                            url:"/item/brand/deleteSome",
                            dataType:"json",
                ...
                layer.msg("请选择需要删除的品牌",{time:1000});
            }
        }
    };
