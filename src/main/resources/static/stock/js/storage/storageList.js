...
layui.config({
    ...
    //监听头工具栏事件
    table.on('toolbar(storageList)', function(obj) {
        var checkStatus = table.checkStatus(obj.config.id),
            data = checkStatus.data; //获取选中的数据
            switch (obj.event) {
                case 'table_export':
                    exportFile('storage-table')
                    break;
                };
            });
    /**
...
//表格导出
                function exportFile(id) {
                    ...
                    $.ajax({
                            type:"POST",
                            url:"/stock/storage/list?limit=999999",
                            dataType:"json",
                            ...
                            success:function(res){
                              ...
                              //  2. 如果需要调整顺序，请执行梳理函数
                                ...
                                excel.exportExcel({
                                    sheet1: data
                                }, '入库单列表' + new Date().toLocaleString() + '.xlsx', 'xlsx');
    ...
    //监听工具条
    table.on('tool(storageList)', function(obj){
        var data = obj.data;
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑入库",
                type : 2,
                content : "/stock/storage/edit?id="+data.id,
        ...
        if(obj.event === "del"){
            layer.confirm("你确定要删除该入库么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/stock/storage/delete",{"id":data.id},function (res){
                        if(res.success){
                            layer.msg("删除成功",{time: 1000},function(){
                                table.reload('storage-table', t);
                            });
                        }else{
        ...
        if(obj.event === "back"){
            layer.confirm("你确定要撤销该入库么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/stock/storage/back",{"id":data.id,"status":1},function (res){
                        if(res.success){
         ...
         if(obj.event === "history"){
           $('#show').html("");//清空上一个
           $.post("/stock/storage/history",{"storageId":data.id},function (res){
    ...
    table.on('rowDouble(storageList)', function(obj){
        var data = obj.data;
        var editIndex = layer.open({
            title : "入库详情",
            type : 2,
    ...
    t = {
        ...
        cols: [[
            {type:'checkbox'},
           /* {field:'id',        title: 'ID'   },*/
            {field:'storageTime',  title: '入库时间',    width:'10%'}, 
