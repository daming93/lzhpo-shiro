layui.config({
    ...
    //监听头工具栏事件
    table.on('toolbar(vehicleContractMainList)', function(obj) {
        var checkStatus = table.checkStatus(obj.config.id),
            data = checkStatus.data; //获取选中的数据
            switch (obj.event) {
                case 'table_export':
                    exportFile('vehicleContractMain-table')
                    break;
                };
            });
    /**
...
//表格导出
                function exportFile(id) {
                    ...
                    $.ajax({
                            type:"POST",
                            url:"/deliver/vehicleContractMain/list?limit=999999",
                            dataType:"json",
    ...
    //监听工具条
    table.on('tool(vehicleContractMainList)', function(obj){
        var data = obj.data;
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑合同",
                type : 2,
                content : "/deliver/vehicleContractMain/edit?id="+data.id,
        ...
        if(obj.event === "del"){
            layer.confirm("你确定要删除该合同么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/deliver/vehicleContractMain/delete",{"id":data.id},function (res){
                        if(res.success){
        ...
        if(obj.event === "back"){
            layer.confirm("你确定要撤销该合同么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/deliver/vehicleContractMain/back",{"id":data.id,"status":1},function (res){
                        if(res.success){
    ...
    table.on('rowDouble(vehicleContractMainList)', function(obj){
        var data = obj.data;
        var editIndex = layer.open({
            title : "合同详情",
            type : 2,
    ...
    t = {
        elem: '#vehicleContractMain-table',
        even: true,
        url:'/deliver/vehicleContractMain/list',
        method:'post',
        toolbar: "#toolbarDemo" ,
        ...
        cols: [[
           ...
           /* {field:'id',        title: 'ID'   },*/
            ...
            {title: '操作',fixed: 'right',  width:'15%',    align: 'center',toolbar: '#vehicleContractMainBar'}
        ]]/*,
        done: function () {
    ...
    var active={
          addUser : function(){
            addIndex = layer.open({
                title : "添加合同单",
                type : 2,
                content : "/deliver/vehicleContractMain/add",
        ...
        //批量删除
        deleteSome : function(){
            var checkStatus = table.checkStatus('vehicleContractMain-table'),
            ...
            if(data.length > 0){
                ...
                layer.confirm("你确定要删除这些合同么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        ...
                        $.ajax({
                            type:"POST",
                            url:"/deliver/vehicleContractMain/deleteSome",
                            dataType:"json",
    ...
    form.on("submit(searchForm)",function(data){
        table.reload('vehicleContractMain-table', {
            page: {curr: 1},
            where: data.field
        });
        return false;
