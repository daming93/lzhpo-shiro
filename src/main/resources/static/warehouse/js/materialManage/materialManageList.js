...
layui.config({
    ...
    //监听头工具栏事件
    table.on('toolbar(materialManageList)', function(obj) {
        var checkStatus = table.checkStatus(obj.config.id),
            data = checkStatus.data; //获取选中的数据
            switch (obj.event) {
                case 'table_export':
                    exportFile('materialManage-table')
                    break;
                };
            });
    /**
...
//表格导出
                function exportFile(id) {
                    ...
                    $.ajax({
                            type:"POST",
                            url:"/warehouse/materialManage/list?limit=999999",
                            dataType:"json",
                            ...
                            success:function(res){
                              ...
                              //  2. 如果需要调整顺序，请执行梳理函数
                                ...
                                excel.exportExcel({
                                    sheet1: data
                                }, '物料调品' + new Date().toLocaleString() + '.xlsx', 'xlsx');
    ...
    $("body").keypress(function(e) {
        if(e.keyCode==10&&e.ctrlKey) {
            $("#addmaterialManage").click();
        }
    });
    //监听工具条
    table.on('tool(materialManageList)', function(obj){
        var data = obj.data;
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑出库",
                type : 2,
                content : "/warehouse/materialManage/edit?id="+data.id,
        ...
        if(obj.event === 'print'){
            window.open("/warehouse/materialManage/print?id="+data.id);
        }
        if(obj.event === "del"){
            layer.confirm("你确定要删除该出库么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/warehouse/materialManage/delete",{"id":data.id},function (res){
                        if(res.success){
                            layer.msg("删除成功",{time: 1000},function(){
                                table.reload('materialManage-table', t);
                            });
                        }else{
        ...
        if(obj.event === "back"){
            layer.confirm("你确定要撤销该出库么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/warehouse/materialManage/back",{"id":data.id,"status":1},function (res){
                        if(res.success){
         ...
         if(obj.event === "history"){
           $('#show').html("");//清空上一个
           $.post("/warehouse/materialManage/history",{"materialManageId":data.id},function (res){
    ...
    table.on('rowDouble(materialManageList)', function(obj){
        var data = obj.data;
        var editIndex = layer.open({
            title : "出库详情",
            type : 2,
    ...
    t = {
        elem: '#materialManage-table',
        even: true,
        url:'/warehouse/materialManage/list',
        method:'post',
        toolbar: "#toolbarDemo" ,
        ...
        cols: [[
            {type:'checkbox'},
           /* {field:'id',        title: 'ID'   },*/
            {field:'time',  title: '转品时间',    width:'10%'}, 
            {field:'clientName',        title: '客户名称'   },
            {field:'clientCode',        title: '客户单号' ,    width:'12%'  },
            {field:'systemCode',        title: '系统单号' ,    width:'13%'   },
         //   {field:'materialManageNumber',        title: '件/托(kg)'   },
            {field:'statusStr',        title: '类型'   },
             {field:'modifyStatusStr',        title: '状态'   },
            {field:'remarks',        title: '备注'   },
            {title: '操作',fixed: 'right',  width:'15%',    align: 'center',toolbar: '#materialManageBar'}
        ]]/*,
        done: function () {
    ...
    var active={
          addUser : function(){
            ...
            var mode = $("input[name='s_mode']:checked").val();
            addIndex = layer.open({
                title : "添加出库单",
                type : 2,
                content : "/warehouse/materialManage/add?continuity="+continuity+"&mode="+mode,
        ...
        //批量删除
        deleteSome : function(){
            var checkStatus = table.checkStatus('materialManage-table'),
            ...
            if(data.length > 0){
                ...
                layer.confirm("你确定要删除这些出库么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        ...
                        $.ajax({
                            type:"POST",
                            url:"/warehouse/materialManage/deleteSome",
                            dataType:"json",
    ...
    form.on("submit(searchForm)",function(data){
        table.reload('materialManage-table', {
            page: {curr: 1},
            where: data.field
        });
        return false;
