...
window.viewObj = {
    ...
    layui.use(['jquery', 'table','upload', 'layer'], function(){
        ...
        var materialManageWidth = $("#materialManageTable").width();

        var tableIns;

        var materialManageIns = table.render({
                    elem: '#materialManageTable',
                    id: 'materialManageTable',
                    width: materialManageWidth,
                    page: false,
                    loading: true,
                    even: false, //不开启隔行背景
...
//提交数据代码
        ...
        data.field.detailSet = table.cache['materialManageTable'];  
        var loadIndex = layer.load(2, {
            shade: [0.3, '#333']
        });
        $.ajax({
            type:"POST",
            url:"/warehouse/materialManage/add",
            dataType:"json",
                ...
                if(res.success){
                    parent.layer.msg("单据添加成功！",{time:1000},function(){
                        ...
                        if(continuity=="true"){//开启
                            var node = parent.document.getElementById("addmaterialManage");
                         //调用该元素的Click事件
                            node.click();//连续录单
                        }else{
                          // 刷新父页面
